...
from .models import login as log, candidate as cad,company as com,job,result as rslt,complaint as cm,feedback as fd,qualificaton as qua,experience as exp,applicant as app, question as qn,invitation as invt,company_feedback as fd_cmp,user_feedback as ufd
...
def listcompany(request):
    datacom=com.objects.filter(status="waiting").all()
...
def company_complaint(request):
    if request.POST:
        t1= request.POST["t1"]
        t2= request.POST["t2"]
        cm.objects.filter(complaint_id=t1).update(complaint_reply=t2)
    data=cm.objects.all()
